...
std::vector<float> WavetableSynth::generateTriWaveTable()
{
    ...
    int halfsize = (WAVETABLE_LENGTH/ 2) - 1; // weil i = 0, starrtet von 0 instead of 1.
    float temp = 0;
    for (auto i = 0; i < WAVETABLE_LENGTH; ++i)
    {
        if (i < halfsize)
...
void WavetableSynth::initializeOscillators()
{
    ...
    {
        currentNotes.push_back(-1); //liste von noten die wir uns merken damit wir note on und note off triggern können
    }
...
void WavetableSynth::render(juce::AudioBuffer<float>& buffer, int startSample, int endSample)
{
    ...
    for (auto& osc : oSC1)
    {
        if (osc.isPlaying())
        {
            ...
            {
                const float temp = gain_OSC1.processSample(osc.getSample() * osc.adsr.getNextSample() * velocity); //temp = 0.0-1.0
    ...
    } // args (range, range, destination); block: startSample-endSample
}
