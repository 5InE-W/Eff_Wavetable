...
public:
    ...
    void prepareToPlay(double sampleRate); //sets the sample rate for processing (analogously to prepareToPlay() from PluginProcessor).
...
private:
    ...
    static std::vector<float> generateSinWaveTable(); //generates 64 samples of a sine wave period.
    ...
    void initializeOscillators(); //initializes z.b. 128 oscillators as wave table oscillators.(check OSCILLATORS_COUNT)
    ...
    const int OSCILLATORS_COUNT = 16; // stimmen, synths voice die gleichtig spielt
    double sampleRate;
    int samplesPerBlock;
